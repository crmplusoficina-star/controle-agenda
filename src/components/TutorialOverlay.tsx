...
export function TutorialOverlay() {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('aria:tutorial:start', restart);
  }, [storageKey]);
  ...
  return <div className="tutorial-layer" role="dialog" aria-modal="true" aria-label="Tutorial da Agenda Técnica">
    ...
    <section className={`tutorial-card tutorial-pos-${index % 3}`}>
      <div className="tutorial-aria-side">
        <img src={images[index % images.length]} alt="ArIA" />
        <span>ArIA</span>
      </div>
      <div className="tutorial-copy">
        <div className="tutorial-topline"><span>Passo {index + 1} de {steps.length}</span><button type="button" onClick={finish}><FastForward size={14}/> Pular tutorial</button></div>
        <h2>{step.title}</h2>
        <p>{text}</p>
        ...
        <div className="tutorial-actions">
          ...
          {last && <>
            ...
            <button type="button" className="tutorial-primary" onClick={finish}><Check size={15}/> Finalizar tutorial</button>
          </>}
        </div>
      </div>
    </section>
