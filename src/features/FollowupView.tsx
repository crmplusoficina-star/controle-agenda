...
export function FollowupView({ rows, loading, onNew }: { rows: Followup[]; loading: boolean; onNew: () => void }) {
  ...
  return <section className="list-page">
    ...
    <div className="table-shell">
      ...
      {loading ? <div className="table-loading">Carregando follow-ups...</div> : filtered.length === 0 ? <EmptyState title="Nenhum follow-up" text="Quando um cliente precisar de retorno, ele aparece aqui — sem misturar com a agenda." /> : filtered.map((item) => <div className="table-row followup-columns" key={item.id}><div><strong>{item.client_name}</strong><small>{item.equipment_serial || item.branch}</small></div><span>{item.treatment_type.replace(/_/g, ' ')}</span><span className={`status-text status-${item.status}`}>{labels[item.status] || item.status}</span><span>{item.next_followup_date ? dateFmt.format(new Date(`${item.next_followup_date}T12:00:00`)) : '—'}</span><strong>{item.estimated_value ? money.format(item.estimated_value) : '—'}</strong></div>)}
    </div>
  </section>;
}
