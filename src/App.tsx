...
export default function App() {
  ...
  async function addTechnician(e: FormEvent) {
    ...
    if (!error) { setTechName(''); setShowTechnician(false); await loadAgenda(); }
  }
  ...
  function newFollowup(client?: ClientSummary) {
    setFollowupDraft({ branch: client?.branch || (branch === ALL ? branches[0]?.name || '' : branch), client_name: client?.client_name || '', equipment_serial: '', action_date: isoDate(new Date()), treatment_type: 'retorno', status: 'contato_realizado', estimated_value: '', next_followup_date: '', notes: '' });
  }
  ...
  return <div className="app-shell">
    ...
    <div className="workspace">
      ...
      <main className="content">
        {view === 'agenda' && <AgendaView weekStart={weekStart} onWeek={(d) => setWeekStart(startOfWeek(d))} technicians={technicians} appointments={appointments} loading={agendaLoading} onNew={openNew} onEdit={openEdit} onAddTechnician={() => setShowTechnician(true)} />}
