...
const USER_AGENT = 'AgendaTecnicaRetencao/5.0';
const PAGE_SIZE = 1000;
const EXACT_GEOCODE_LIMIT = 8;
const ROUTE_RADIUS_KM = 30;

const stateNames: Record<string, string> = {
  AC:'Acre',AL:'Alagoas',AP:'Amapá',AM:'Amazonas',BA:'Bahia',CE:'Ceará',DF:'Distrito Federal',ES:'Espírito Santo',GO:'Goiás',MA:'Maranhão',MT:'Mato Grosso',MS:'Mato Grosso do Sul',MG:'Minas Gerais',PA:'Pará',PB:'Paraíba',PR:'Paraná',PE:'Pernambuco',PI:'Piauí',RJ:'Rio de Janeiro',RN:'Rio Grande do Norte',RS:'Rio Grande do Sul',RO:'Rondônia',RR:'Roraima',SC:'Santa Catarina',SP:'Santa Catarina',SE:'Sergipe',TO:'Tocantins'
};
...
type Candidate={id:string;kind:'branch'|'appointment'|'client';query?:string;cityQuery?:string;cityName?:string;state?:string;payload:Record<string,unknown>};
...
function haversineKm(a:[number,number],b:[number,number]){const r=(x:number)=>x*Math.PI/180,e=6371,dlat=r(b[0]-a[0]),dlng=r(b[1]-a[1]),l1=r(a[0]),l2=r(b[0]);const h=Math.sin(dlat/2)**2+Math.cos(l1)*Math.cos(l2)*Math.sin(dlng/2)**2;return 2*e*Math.asin(Math.min(1,Math.sqrt(h)))}
function distanceToRouteKm(point:[number,number],geometry:[number,number][]) { if(!geometry.length)return Infinity; const step=Math.max(1,Math.floor(geometry.length/280)); let min=Infinity; for(let i=0;i<geometry.length;i+=step){min=Math.min(min,haversineKm(point,geometry[i])); if(min<=1)return min;} return Math.min(min,haversineKm(point,geometry[geometry.length-1])); }
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    for(const a of appointments){const base=a.client_name?byClientBranch.get(`${normalize(a.client_name)}|${normalize(a.branch)}`):undefined; const city=String(a.service_city||base?.city||'').trim(); const state=String(base?.state||'').trim(); candidates.push({id:`appointment:${a.id}`,kind:'appointment',query:buildExact(base?.address,base?.neighborhood,city,state)||undefined,cityQuery:buildCity(city,state)||undefined,cityName:city||undefined,state:state||undefined,payload:{...a,city:city||null,state:state||null}});}
    ...
    for(const c of candidates)if(c.cityQuery&&c.cityName&&c.state)cityCandidates.set(c.cityQuery,c);
    ...
    const points:any[]=[];
    for(const c of candidates){const loc=await resolve(c); if(!loc)continue; points.push({id:c.id,kind:c.kind,lat:Number(loc.lat),lng:Number(loc.lng),precision:loc.precision||'city',location_source:loc.source||null,location_label:loc.display_name||null,...c.payload});}

    let route:any=null;
    if(selectedTechnicianId){
      const stops=selectedAppointments.map(a=>points.find(p=>p.kind==='appointment'&&p.id===`appointment:${a.id}`)).filter(Boolean);
      ...
      if(routePoints.length>=2){
        ...
        if(route.geometry.length){let nearby=0;for(const p of points){if(p.kind!=='client')continue;const d=distanceToRouteKm([p.lat,p.lng],route.geometry);p.route_distance_km=Math.round(d*10)/10;p.near_route=d<=ROUTE_RADIUS_KM;if(p.near_route)nearby++;}route.nearby_clients=nearby;route.radius_km=ROUTE_RADIUS_KM;}
      }
    }

    const clientPoints=points.filter(p=>p.kind==='client');
    return new Response(JSON.stringify({points,route,unresolved:Math.max(0,clients.length-clientPoints.length),geocoded_now:geocodedNow,exact_external_calls:exactCalls,requested_clients:clients.length,located_clients:clientPoints.length}),{headers:{...corsHeaders,'Content-Type':'application/json','Cache-Control':'no-store'}});
